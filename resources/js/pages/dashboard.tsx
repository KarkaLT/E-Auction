...
export default function Dashboard() {
    const { props } = usePage<{ auctionItems?: DashboardPageProps['auctionItems'] }>();
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
                <div className="flex items-center justify-between">
                    ...
                    <Link href={auctionItems.create().url} className={buttonVariants({ variant: 'default' })}>Create Auction</Link>
                </div>
                ...
                {auctionItemsList.length > 0 && (
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {auctionItemsList.map(item => (
                            <div key={item.id} className="group rounded-lg border bg-card p-4 shadow-sm transition hover:shadow-md flex flex-col justify-between">
                                <div>
                                    <div className="mb-2 flex items-center justify-between gap-2">
                                        <Link
                                            href={auctionItems.show(item.id).url}
                                            className="text-base font-medium group-hover:text-primary line-clamp-1 flex-1"
                                        >
                                            {item.title}
                                        </Link>
                                ...
                                <Form
                                    method="post"
                                    action={auctionItems.destroy(item.id).url + '?from_dashboard=1'}
